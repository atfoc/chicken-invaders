...
void on_thread(rg::engine::uuid scene_id, int n);
...
int main(int argc, char *argv[])
{
	...
	rg::engine::application::create_thread(on_thread, s->id(), 10);
...
void on_thread(rg::engine::uuid scene_id, int n)
{
	...
	for(int i = 0; i < n; ++i)
	{
		...
		rg::engine::application::post_event(rotate_event(scene_id));
